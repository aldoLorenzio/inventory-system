...
describe('orderItem routes', () => {
  ...
  describe('GET & POST /v1/orderItem', () => {
    test('should return 200 OK if success get orderItems', async () => {
      await request(app).get('/v1/orderItem').set('Authorization', `Bearer ${adminAccessToken}`).expect(httpStatus.OK);
    });
  ...
  describe('ERROR TEST', () => {
    ...
    test('should return 401 UNAUTHORIZED if no access token', async () => {
      await request(app).get(`/v1/orderItem`).send(newOrderItem).expect(httpStatus.UNAUTHORIZED);
    });
