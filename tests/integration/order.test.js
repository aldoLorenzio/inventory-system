...
describe('Order routes', () => {
  ...
  describe('POST & GET /v1/order', () => {
    test('should return 200 OK if success get orders', async () => {
      await request(app).get('/v1/order').set('Authorization', `Bearer ${adminAccessToken}`).expect(httpStatus.OK);
    });
  ...
  describe('ERROR TEST', () => {
    ...
    test('should return 401 UNAUTHORIZED if no access token', async () => {
      await request(app).get(`/v1/order`).send(newOrder).expect(httpStatus.UNAUTHORIZED);
    });
