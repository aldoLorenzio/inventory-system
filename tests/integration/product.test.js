...
describe('product routes', () => {
  ...
  describe('GET & POST /v1/product', () => {
    test('should return 200 OK if success get products', async () => {
      await request(app).get('/v1/product').set('Authorization', `Bearer ${userOneAccessToken}`).expect(httpStatus.OK);
    });
  ...
  describe('ERROR TEST', () => {
    ...
    test('should return 401 UNAUTHORIZED if no access token', async () => {
      await request(app).get(`/v1/product`).send(newProduct).expect(httpStatus.UNAUTHORIZED);
    });
